...
/*	public boolean inserirUsuario(Usuario usuario) {
	...
	public boolean inserirCurso(Curso curso) {
		...
		try {  
			Statement st = conexao.createStatement();
			st.executeUpdate("INSERT INTO curso (id, nome) "
					       + "VALUES ("+curso.getID()+ ", '" + curso.getNome() + "');");
			st.close();
			status = true;
		} catch (SQLException u) {  
			throw new RuntimeException(u);
...
/*	public boolean atualizarUsuario(Usuario usuario) {
	...
	public boolean atualizarCurso(Curso curso) {
		...
		try {  
			Statement st = conexao.createStatement();
			String sql = "UPDATE curso SET nome = '" + curso.getNome() + "' WHERE codigo = " + curso.getID();
			st.executeUpdate(sql);
			st.close();
			status = true;
		} catch (SQLException u) {  
		...
		try {
			...
			ResultSet rs = st.executeQuery("SELECT * FROM curso");		
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                curso[i] = new Curso(rs.getInt("id"), rs.getString("nome"));
	             }
	          }
	          st.close();
		} catch (Exception e) {
